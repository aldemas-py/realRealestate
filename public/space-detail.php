...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['request_visit'])) {
    ...
    if ($pendingCount->fetchColumn() >= $maxPending) $errors[] = "You can only have $maxPending pending visit requests.";
    if (empty($errors)) {
        try {
            $stmt = $db->prepare("INSERT INTO visit_requests (user_id, space_id, preferred_date, preferred_time, notes) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$_SESSION['user_id'], $space['id'], $preferredDate, $preferredTime, $notes]);
            logAudit($_SESSION['user_id'], 'visit.request', 'visit_request', $db->lastInsertId(), null, ['space_id' => $space['id'], 'date' => $preferredDate], 'pass');
            $_SESSION['success'] = 'Visit request submitted! The admin will confirm your appointment.';
        } catch (Exception $e) {
            if ($e->getCode() == 23000) {
                $_SESSION['error'] = 'You already have a pending request for this space.';
            } else {
...
$pageTitle = $space['name'] . ' - Zahara Co-Working Space';
            ...
            <div>
                <div class="space-info">
                    <div class="space-meta">
                        ...
                        <?php if ($space['size_sqft'] > 0): ?><span>&#128207; <?= $space['size_sqft'] ?> sq
                                ft</span><?php endif; ?>
                    </div>
                    <h1><?= htmlspecialchars($space['name']) ?></h1>
                    <div class="space-price">KES <?= number_format($space['price_per_month']) ?> <small>/month</small>
                    </div>
                    <p style="color: var(--text-mid); font-size: 0.9rem;">Security deposit: KES
                        <?= number_format($space['security_deposit']) ?></p>
                    <?php if (!empty($amenities)): ?>
                        <div class="space-amenities"><?php foreach ($amenities as $amenity): ?><span
                                    class="amenity-tag"><?= ucwords(str_replace('_', ' ', $amenity)) ?></span><?php endforeach; ?>
                        </div>
                    <?php endif; ?>
                    <div class="space-description-full"><?= nl2br(htmlspecialchars($space['description'])) ?></div>
                    <div class="request-form">
                        <h3>Request a Site Visit</h3>
                        <p style="font-size: 0.85rem; color: var(--text-mid); margin-bottom: 16px;">See the space in
                            person at Krishna Centre, Westlands.</p>
                        <form method="POST" action="">
                            <div class="form-group"><label for="preferred_date">Preferred Date</label><input type="date"
                                    id="preferred_date" name="preferred_date"
                                    min="<?= date('Y-m-d', strtotime('+1 day')) ?>" required></div>
                            <div class="form-group"><label for="preferred_time">Preferred Time</label><input type="time"
                                    id="preferred_time" name="preferred_time" required></div>
                            <div class="form-group"><label for="notes">Additional Notes (optional)</label><textarea
                                    id="notes" name="notes"
                                    placeholder="Any special requirements or questions..."></textarea></div>
                            <button type="submit" name="request_visit" class="btn btn-primary"
                                style="width: 100%; justify-content: center;">Request Visit</button>
                        </form>
                    </div>
                </div>
                <div class="map-container" id="spaceMap" style="margin-top: 24px;"></div>
</section>
<script>
    document.addEventListener('DOMContentLoaded', function() {
        if (typeof initSingleSpaceMap === 'function') {
            initSingleSpaceMap('spaceMap', {
                lat: <?= $space['latitude'] ?>,
                lng: <?= $space['longitude'] ?>,
                name: '<?= addslashes($space['name']) ?>',
                address: '<?= addslashes($space['address_line1'] . ', ' . $space['city']) ?>'
            });
        }
    });
</script>
