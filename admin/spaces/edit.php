...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $db->prepare("
            UPDATE office_spaces SET name=?, slug=?, description=?, short_description=?, space_type=?, capacity=?, 
            price_per_month=?, security_deposit=?, address_line1=?, city=?, country=?, latitude=?, longitude=?, 
            size_sqft=?, amenities=?, is_featured=? WHERE id=?
        ");
        $stmt->execute([
            $name,
            $slug,
...
<div class="admin-layout">
    ...
    <main class="admin-main">
        <div class="admin-header">
            <h1>Edit: <?= htmlspecialchars($space['name']) ?></h1>
        ...
        <form method="POST" action="" style="max-width: 800px;">
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                <div class="form-group" style="grid-column: 1/-1;">
                    ...
                    <input type="text" id="name" name="name" required value="<?= htmlspecialchars($space['name']) ?>">
                </div>
                <div class="form-group" style="grid-column: 1/-1;">
                    <label for="slug">URL Slug</label>
                    <input type="text" id="slug" name="slug" value="<?= htmlspecialchars($space['slug']) ?>">
                </div>
                <div class="form-group" style="grid-column: 1/-1;">
                    <label for="description">Full Description</label>
                    <textarea id="description" name="description"
                        rows="5"><?= htmlspecialchars($space['description']) ?></textarea>
                </div>
                <div class="form-group" style="grid-column: 1/-1;">
                    ...
                    <input type="text" id="short_description" name="short_description" maxlength="300"
                        value="<?= htmlspecialchars($space['short_description']) ?>">
                </div>
                <div class="form-group">
                    <label for="space_type">Space Type *</label>
                    <select id="space_type" name="space_type" required>
                        <?php foreach (['private_office', 'open_desk', 'meeting_room', 'virtual_office'] as $t): ?>
                            <option value="<?= $t ?>" <?= $space['space_type'] === $t ? 'selected' : '' ?>>
                                <?= ucwords(str_replace('_', ' ', $t)) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="capacity">Capacity *</label>
                    <input type="number" id="capacity" name="capacity" min="1" value="<?= $space['capacity'] ?>"
                        required>
                </div>
                <div class="form-group">
                    <label for="price_per_month">Price/Month (KES) *</label>
                    <input type="number" id="price_per_month" name="price_per_month" min="0" step="0.01"
                        value="<?= $space['price_per_month'] ?>" required>
                </div>
                <div class="form-group">
                    <label for="security_deposit">Deposit (KES)</label>
                    <input type="number" id="security_deposit" name="security_deposit" min="0" step="0.01"
                        value="<?= $space['security_deposit'] ?>">
                </div>
                <div class="form-group">
                    <label for="address_line1">Address *</label>
                    <input type="text" id="address_line1" name="address_line1" required
                        value="<?= htmlspecialchars($space['address_line1']) ?>">
                </div>
                <div class="form-group">
                    <label for="city">City</label>
                    <input type="text" id="city" name="city" value="<?= htmlspecialchars($space['city']) ?>">
                </div>
                <div class="form-group">
                    <label for="country">Country</label>
                    <input type="text" id="country" name="country" value="<?= htmlspecialchars($space['country']) ?>">
                </div>
                <div class="form-group">
                    <label for="latitude">Latitude</label>
                    <input type="number" id="latitude" name="latitude" step="0.0001" value="<?= $space['latitude'] ?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="size_sqft" name="size_sqft" min="0" value="<?= $space['size_sqft'] ?>">
                </div>
                <div class="form-group">
                    <label>
                        <input type="checkbox" name="is_featured" value="1"
                            <?= $space['is_featured'] ? 'checked' : '' ?>> Featured
                    </label>
                </div>
                    ...
                    <select name="amenities[]" multiple style="height: 120px;">
                        ...
                        foreach ($allAmenities as $a): ?>
                            <option value="<?= $a ?>" <?= in_array($a, $amenities) ? 'selected' : '' ?>>
                                <?= ucwords(str_replace('_', ' ', $a)) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div style="display: flex; gap: 8px;">
                    ...
                    <a href="/work_folder/realRealestate/admin/spaces/index.php" class="btn btn-ghost">Cancel</a>
                </div>
        </form>
    </main>
</div>
