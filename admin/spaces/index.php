...
$spaces = $db->query("
    SELECT os.*, 
           ...
           (SELECT COUNT(*) FROM leases WHERE space_id = os.id AND status IN ('active','deposit_pending')) as active_leases_count
...
<div class="admin-layout">
    ...
    <main class="admin-main">
        <div class="admin-header">
            <h1>Manage Office Spaces</h1>
            <a href="/work_folder/realRealestate/admin/spaces/create.php" class="btn btn-primary">+ Add New Space</a>
        </div>

        <?php if (empty($spaces)): ?>
            <div style="text-align: center; padding: 60px; color: var(--text-light);">
                <p style="font-size: 1.1rem;">No spaces created yet.</p>
                <a href="/work_folder/realRealestate/admin/spaces/create.php" class="btn btn-primary"
                        ...
                        <?php foreach ($spaces as $s): ?>
                            <tr>
                                <td>
                                    ...
                                    <small style="color: var(--text-light);"><?= htmlspecialchars($s['address_line1']) ?>,
                                        <?= htmlspecialchars($s['city']) ?></small>
                                </td>
                                ...
                                <td>
                                    <div style="display: flex; gap: 4px; flex-wrap: wrap;">
                                        <a href="/work_folder/realRealestate/admin/spaces/edit.php?id=<?= $s['id'] ?>"
                                            class="btn btn-sm btn-outline">Edit</a>
                                        <a href="/work_folder/realRealestate/public/space-detail.php?slug=<?= $s['slug'] ?>"
                                            class="btn btn-sm btn-ghost" target="_blank">View</a>
                                        <a href="?toggle_status=<?= $s['status'] === 'available' ? 'maintenance' : 'available' ?>&id=<?= $s['id'] ?>"
                                            class="btn btn-sm btn-ghost">Toggle</a>
                                        <a href="?delete=<?= $s['id'] ?>" class="btn btn-sm btn-danger"
                                            data-confirm="Delete this space? This cannot be undone.">Delete</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
